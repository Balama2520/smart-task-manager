...
var pomPhase = 'focus'; // 'focus' | 'short' | 'long'
var pomSessionCount = parseInt(lsGet('pomSessionCount', '0'), 10) || 0;
var POM_DURATIONS = { focus: 25 * 60, short: 5 * 60, long: 15 * 60 };
...
function render() {
  ...
  var shown = tasks.filter(function (t) {
    ...
    else if (currentFilter === 'today') matchFilter = t.due === today && !t.done;
    else if (currentFilter === 'overdue') matchFilter = !!t.due && t.due < today && !t.done;
    else matchFilter = t.category === currentFilter;
...
function drawTask(t) {
  ...
  return '<div class="task-item' + (t.done ? ' done' : '') + '" data-id="' + t.id + '">' +
    '<span class="dragHandle" data-a="handle" aria-hidden="true" title="Drag to reorder">⠿</span>' +
    ...
    '</div>' +
    '<div class="tactions">' +
    '<button class="tabtn" data-a="up" title="Move up" aria-label="Move task up: ' + esc(t.text) + '">↑</button>' +
    '<button class="tabtn" data-a="down" title="Move down" aria-label="Move task down: ' + esc(t.text) + '">↓</button>' +
...
function attachEvents() {
  document.querySelectorAll('.task-item').forEach(function (item) {
    ...
    item.querySelector('[data-a="up"]').onclick = function () { moveTask(id, -1); };
    item.querySelector('[data-a="down"]').onclick = function () { moveTask(id, 1); };

    // Reordering via the grip handle, using Pointer Events so the exact same
    // code path works for mouse *and* touch (native HTML5 drag/drop never
    // fires on touch screens, which used to make this feature mouse-only).
    var handle = item.querySelector('[data-a="handle"]');
    if (handle) {
      handle.addEventListener('pointerdown', function (e) {
        startHandleDrag(e, item, id);
      });
    }
  });
}

// ── Move up / down (keyboard- and screen-reader-accessible reordering) ────
...
// ── Pointer-based drag reordering (mouse + touch + pen, one code path) ─────
var dragState = null;

function startHandleDrag(e, item, id) {
  e.preventDefault();
  dragState = { id: id, item: item, overId: null, pointerId: e.pointerId };
  item.classList.add('dragging');
  try { e.target.setPointerCapture(e.pointerId); } catch (err) { /* unsupported — drag still works via listeners below */ }
...
function onHandleDragMove(e) {
  ...
  var over = el ? el.closest('.task-item') : null;

  document.querySelectorAll('.task-item.drag-over').forEach(function (x) { x.classList.remove('drag-over'); });
  if (over && over !== dragState.item) {
    over.classList.add('drag-over');
    dragState.overId = over.getAttribute('data-id');
  } else {
...
function onHandleDragEnd() {
  ...
  if (dragState.overId && dragState.overId !== dragState.id) {
    reorderTasks(dragState.id, dragState.overId);
    if (navigator.vibrate) navigator.vibrate(12); // subtle haptic tick on drop, phones only
  }
  dragState = null;
}
...
// ── Inline edit — text, category, priority and due date all editable ──────
function startEdit(id, item) {
  ...
  catSel.className = 'tsel tedit-sel';
  [['work', '💼 Work'], ['study', '📚 Study'], ['personal', '🏠 Personal'], ['other', '📌 Other']]
    .forEach(function (o) {
      var opt = document.createElement('option');
      opt.value = o[0]; opt.textContent = o[1];
      if (o[0] === t.category) opt.selected = true;
      catSel.appendChild(opt);
    });

  var priSel = document.createElement('select');
  priSel.className = 'tsel tedit-sel';
  [['high', '🔴 High'], ['medium', '🟡 Medium'], ['low', '🟢 Low']].forEach(function (o) {
    var opt = document.createElement('option');
    opt.value = o[0]; opt.textContent = o[1];
    if (o[0] === t.priority) opt.selected = true;
    priSel.appendChild(opt);
  });

  var dateInp = document.createElement('input');
  dateInp.type = 'date';
  dateInp.className = 'tsel tedit-sel';
  dateInp.value = t.due || '';

  wrap.appendChild(inp);
  wrap.appendChild(catSel);
  wrap.appendChild(priSel);
  ...
  function commit() {
    ...
    committed = true;
    if (inp.value.trim()) {
      t.text = inp.value.trim();
      t.category = catSel.value;
      t.priority = priSel.value;
      t.due = dateInp.value || '';
      save();
    }
    render();
  ...
  // Commit once focus leaves the whole edit group (not on every field blur),
  // so tabbing between the text/category/priority/date fields doesn't cancel the edit.
...
function startVoice() {
  ...
  var rec = new SR();
  // Use the browser/OS's own language rather than a hardcoded locale, so
  // voice input actually understands users outside en-IN correctly.
...
// ── Pomodoro (focus → short break ×3 → long break, repeating) ─────────────
function updateTimerDisplay() {
  ...
  var label = document.getElementById('pomPhaseLabel');
  if (label) {
    label.textContent = pomPhase === 'focus' ? '🍅 Focus'
      : pomPhase === 'short' ? '☕ Short break'
        : '🛋 Long break';
  }
  var count = document.getElementById('pomSessionCountLbl');
  if (count) count.textContent = pomSessionCount + (pomSessionCount === 1 ? ' session' : ' sessions') + ' completed';
}

function notifyPomComplete(title, body) {
  toast(title);
  if ('Notification' in window && Notification.permission === 'granted') {
    try { new Notification(title, { body: body, icon: 'icon-192.png' }); } catch (e) { /* ignore */ }
  }
}
...
function advancePomPhase() {
  if (pomPhase === 'focus') {
    pomSessionCount++;
    lsSet('pomSessionCount', String(pomSessionCount));
    var isLong = pomSessionCount % 4 === 0;
    pomPhase = isLong ? 'long' : 'short';
    notifyPomComplete('🍅 Focus session complete!', isLong ? 'Time for a long break.' : 'Time for a short break.');
  } else {
    var wasLong = pomPhase === 'long';
    pomPhase = 'focus';
    notifyPomComplete(wasLong ? '🛋 Long break over' : '☕ Break over', 'Ready for another focus session?');
  }
  timerSecs = POM_DURATIONS[pomPhase];
  updateTimerDisplay();
}
...
var VALID_CATS = { work: 1, study: 1, personal: 1, other: 1 };
var VALID_PRIS = { high: 1, medium: 1, low: 1 };

// Coerce arbitrary imported JSON into well-formed task objects so a bad or
// hand-edited file can't corrupt the list or crash render()/sort/search.
function sanitizeImportedTask(raw) {
  ...
  return {
    ...
    text: text,
    category: VALID_CATS[raw.category] ? raw.category : 'other',
    priority: VALID_PRIS[raw.priority] ? raw.priority : 'medium',
    due: typeof raw.due === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(raw.due) ? raw.due : '',
    done: raw.done === true
  };
}

// Shared by the manual Import button and the OS-level File Handler launch path
// (double-clicking an exported tasks-*.json once the app is installed).
function applyImportedTaskData(data, sourceLabel) {
  if (!Array.isArray(data)) { toast('Invalid file — expected a task list'); return; }

  var clean = data.map(sanitizeImportedTask).filter(Boolean);
  if (clean.length === 0) { toast('No valid tasks found in file'); return; }

  var shouldMerge = tasks.length > 0 &&
    confirm(clean.length + ' tasks found' + (sourceLabel ? ' in ' + sourceLabel : '') +
      '.\n\nOK = merge with your ' + tasks.length + ' existing tasks\nCancel = replace all existing tasks with the imported ones');

  var prevTasks = tasks;
  if (shouldMerge) {
    // Merge, skipping ids that already exist so re-importing a file is safe.
    var existingIds = {};
    tasks.forEach(function (t) { existingIds[t.id] = true; });
    tasks = tasks.concat(clean.filter(function (t) { return !existingIds[t.id]; }));
  } else {
    tasks = clean;
  }
  save();
  render();
  toast(clean.length + ' tasks imported', 'Undo', function () {
    tasks = prevTasks;
    save();
    render();
  });
...
function importTasks() {
  ...
  inp.onchange = function (e) {
    ...
    reader.onload = function (ev) {
      ...
      } catch (err) { toast('Invalid file — not valid JSON'); return; }
      applyImportedTaskData(data, file.name);
    };
...
// ── File Handler (manifest.json "file_handlers") ───────────────────────────
// Lets an installed copy of the app open an exported tasks-*.json file
// directly from the OS (e.g. double-click, or "Open with → Smart Tasks").
if ('launchQueue' in window) {
  window.launchQueue.setConsumer(async function (launchParams) {
    ...
    try {
      ...
      toast('Could not open that file — make sure it\'s a Smart Tasks export');
    }
  });
}
...
// Wipes everything the app has stored on this device — tasks, name, theme,
// streak, reminder settings. The privacy policy states this data can be
// fully removed at any time; this gives that a real in-app control instead
// of requiring the user to clear browser site data manually.
function clearAllData() {
  var sure = confirm('Delete ALL local data?\n\nThis removes every task, your name, theme, streak, and reminder settings from this device. This cannot be undone.');
  if (!sure) return;
...
// Each day of use writes a fresh 'notified_YYYY-MM-DD' key that's never read
// again after that day — left unchecked these accumulate forever. Prune
// anything older than a week whenever we run the reminder check.
function pruneStaleNotifiedKeys() {
  try {
    var cutoff = new Date();
    cutoff.setDate(cutoff.getDate() - 7);
    for (var i = localStorage.length - 1; i >= 0; i--) {
      var key = localStorage.key(i);
      if (!key || key.indexOf('notified_') !== 0) continue;
      var datePart = key.slice('notified_'.length);
...
// ── Window Controls Overlay (desktop installed PWAs) ────────────────────────
if ('windowControlsOverlay' in navigator) {
  var applyWcoState = function () {
    document.body.classList.toggle('wco-active', navigator.windowControlsOverlay.visible);
  };
  applyWcoState();
  navigator.windowControlsOverlay.addEventListener('geometrychange', applyWcoState);
}

// ── Background Sync / Periodic Sync registration ────────────────────────────
// Both stay fully local — see sw.js for what each tag actually does (or, for
// periodic sync, doesn't do beyond waking the worker on a schedule).
if ('serviceWorker' in navigator) {
  navigator.serviceWorker.ready.then(function (reg) {
    if ('sync' in reg) {
      reg.sync.register('smart-tasks-reminder-check').catch(function () { /* unsupported or denied — ignore */ });
    }
    if ('periodicSync' in reg) {
      navigator.permissions.query({ name: 'periodic-background-sync' }).then(function (status) {
        if (status.state === 'granted') {
          reg.periodicSync.register('smart-tasks-periodic-check', { minInterval: 24 * 60 * 60 * 1000 })
            .catch(function () { /* browser declined — ignore */ });
        }
      }).catch(function () { /* permissions API doesn't know this name — ignore */ });
    }
  });
}
...
function installApp() {
  ...
  deferredInstallPrompt.userChoice.then(function (choice) {
    if (choice.outcome === 'accepted') toast('Installing Smart Tasks…');
...
window.addEventListener('appinstalled', function () {
  var btn = document.getElementById('installBtn');
  if (btn) btn.style.display = 'none';
  toast('Installed! Find Smart Tasks on your home screen');
});
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Clear all data (in profile modal)
  ...
  // Pomodoro display (phase label + session count) before first Start press
  updateTimerDisplay();

  // App shortcuts (manifest.json "shortcuts" — Android long-press home-screen menu)
  var launchAction = new URLSearchParams(window.location.search).get('action');
  if (launchAction === 'add') {
    var ti0 = document.getElementById('taskInput');
    if (ti0) setTimeout(function () { ti0.focus(); }, 300);
  } else if (launchAction === 'pomodoro') {
    var pc0 = document.getElementById('pomCard');
    if (pc0) pc0.style.display = 'block';
  }

  // Protocol Handler (manifest.json "protocol_handlers") — an installed copy
  // can be launched as the target of web+smarttasks:// links, e.g.
  // web+smarttasks:Buy%20milk opens the app with ?task=Buy%20milk.
  var protoTask = new URLSearchParams(window.location.search).get('task');
  if (protoTask) {
    var decoded = protoTask.replace(/^web\+smarttasks:/, '').trim();
    if (decoded) {
      tasks.unshift({
        id: makeId(),
        createdAt: Date.now(),
        text: decoded.slice(0, 300),
        category: 'other',
        priority: 'medium',
        due: '',
        done: false
      ...
      save();
      render();
      toast('Task added from link ✓');
      window.history.replaceState({}, '', window.location.pathname);
    }
  }

  // Share Target (manifest.json "share_target") — lets someone share text,
  // a link, or a title from another app straight into a new task here.
  var shareParams = new URLSearchParams(window.location.search);
  var sharedTitle = shareParams.get('share_title') || '';
  var sharedText = shareParams.get('share_text') || '';
  var sharedUrl = shareParams.get('share_url') || '';
  var sharedCombined = [sharedTitle, sharedText, sharedUrl].filter(Boolean).join(' — ').trim();
  if (sharedCombined) {
    tasks.unshift({
      id: makeId(),
      createdAt: Date.now(),
      text: sharedCombined.slice(0, 300),
      category: 'other',
      priority: 'medium',
      due: '',
      done: false
    ...
    save();
    render();
    toast('Task added from share ✓');
    // Strip the share params from the URL so a page refresh doesn't re-add it.
    window.history.replaceState({}, '', window.location.pathname);
  }
